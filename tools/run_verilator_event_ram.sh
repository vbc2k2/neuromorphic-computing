#!/usr/bin/env bash
# ============================================================================
# run_verilator_event_ram.sh - Verilator run for the area-oriented v3 event SNN.
#
# Usage:
#   bash tools/run_verilator_event_ram.sh [first] [last] [tag] [max_cycles_per_image]
# ============================================================================
set -euo pipefail
...
SIM="$ROOT/sim"
RTL="$ROOT/rtl"
RUNDIR="$SIM/run_verilator_event_ram${TAG}"
...
echo "[verilator-event-ram$TAG] build  ($(date))"
cd "$ROOT"
...
verilator -sv --cc \
    ...
    -GFIFO_DEPTH=4096 \
    "$RTL/synapse_csr.sv" \
    "$RTL/top_event_ram.sv" \
    --top-module top_event_ram \
    --exe "$ROOT/tools/verilator_event_main.cpp" \
    -CFLAGS "-std=c++17 -DEVENT_TOP_HEADER=\\\"Vtop_event_ram.h\\\" -DEVENT_TOP_CLASS=Vtop_event_ram -DEVENT_DESIGN_NAME=\\\"event_ram_verilator\\\" -DSNN_N_TOTAL=$N_TOTAL -DSNN_N_INPUT=$N_INPUT -DSNN_N_OUTPUT=$N_OUTPUT -DSNN_ID_BIAS=$ID_BIAS -DSNN_ID_OUTPUT_BASE=$OUT_BASE -DSNN_T_STEPS=$T_STEPS"
make -C "$OBJDIR" -f Vtop_event_ram.mk -j "${VERILATOR_JOBS:-1}"

echo "[verilator-event-ram$TAG] images $FIRST..$LAST  ($(date))"
cd "$RUNDIR"
"$OBJDIR/Vtop_event_ram" \
...
echo "[verilator-event-ram$TAG] done  ($(date))"
